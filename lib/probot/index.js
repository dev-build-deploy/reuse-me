 ...
 * @param context
 */
const onPush = (context) => __awaiter(void 0, void 0, void 0, function* () {
    github.setRepositoryContext(context.repo.owner, context.repo.repo, context.payload.ref);
    const datasource = new datasources_1.CommitsSource(context.octokit, context.payload.commits);
    const issues = yield github.getIssues(context);
    for (let file of yield datasource.getChangedFiles()) {
        ...
        yield github.updateIssue(context, issues, file, header);
    }
});
module.exports = (app) => {
    app.on('push', onPush);
