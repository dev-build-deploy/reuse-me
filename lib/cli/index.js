...
*/
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.licenseMap = exports.wildcardMatch = exports.load = void 0;
 ...
 */
function parseFileStanza(stanza) {
    const filesStanza = {
        files: [],
        license: "",
        copyright: [],
 ...
 */
async function licenseMap(debianPackage, files) {
    ...
    for (const packageFiles of debianPackage.files) {
        for (const patternFile of packageFiles.files) {
            files
                ...
                .forEach(async (file) => {
                fs.writeFileSync(`${(0, utils_1.getSourceFile)(file)}.deb5`, `${packageFiles.copyright
                    .map(copyright => `SPDX-FileCopyrightText: ${copyright}`)
                    .join("\n")}${packageFiles.license ? `\nSPDX-License-Identifier: ${packageFiles.license}` : ""}`);
                fileMap.set((0, utils_1.getSourceFile)(file), await spdx.parseFile(`${(0, utils_1.getSourceFile)(file)}.deb5`));
            });
        }
    }
    return fileMap;
...
const debian = __nccwpck_require__(974);
const crypto = __nccwpck_require__(113);
const utils_1 = __nccwpck_require__(314);
const commentIt = __nccwpck_require__(700);
/**
 * SPDX Document
...
class SoftwareBillOfMaterials {
     ...
     */
    async createFile(file, debianLicenseMap) {
        ...
        if (hasValidLicense(licenseFileLicense) || hasValidCopyrightText(licenseFileLicense))
    ...
    async gatherFiles() {
        ...
        const debianLicenseMap = debianConfig ? await debian.licenseMap(debianConfig, changedFiles) : new Map();
        // Validate each file asynchronously
        return await Promise.all(changedFiles
...
async function parseFile(fileName) {
    ...
    if (!commentIt.isSupported(fileName))
        return file;
    for await (const comment of commentIt.extractComments(fileName, { maxLines: 20 })) {
        ...
        for (const match of copyrightMatches) {
            if ((match === null || match === void 0 ? void 0 : match.groups) === undefined)
                continue;
            if (file.copyrightText !== undefined)
                continue;
            if (SPDXCopyrightIdentifierRegex.test(match.groups.identifier) === false)
                continue;
...
program
    ...
    const sbom = new spdx_1.SoftwareBillOfMaterials(await datasource.getRepositoryName(), datasource);
    await sbom.generate();
